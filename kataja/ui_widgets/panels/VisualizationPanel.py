...
class VisualizationPanel(Panel):
    ...
    def __init__(self, name, default_position='right', parent=None, folded=False):
        ...
        hlayout = QtWidgets.QHBoxLayout()
        hlayout.setContentsMargins(0, 0, 0, 0)
        gb = QtWidgets.QGroupBox(self)
        gb.setFlat(True)
        #gb.setContentsMargins(0, 0, 0, 0)
        #bg = QtWidgets.QButtonGroup()
        gb.setTitle('Node shape')
        #gb.setContentsMargins(0, 0, 0, 0)
        #layout.setContentsMargins(0, 0, 0, 0)
        w = 36
        #bg.setExclusive(True)
        b1 = QtWidgets.QPushButton('N', gb)
        b1.setFixedWidth(w)
        #b1 = PanelButton(text='N', parent=gb)
        b1.setCheckable(True)
        b1.setFlat(False)
        #bg.addButton(b1)
        ctrl.ui.connect_element_to_action(b1, 'set_no_frame_node_shape')
        hlayout.addWidget(b1)
        b2 = QtWidgets.QPushButton('[N__]', gb)
        b2.setFixedWidth(w)
        #b2 = PanelButton(text='[N__]', parent=gb)
        b2.setCheckable(True)
        b2.setFlat(False)
        #bg.addButton(b2)
        ctrl.ui.connect_element_to_action(b2, 'set_scopebox_node_shape')
        hlayout.addWidget(b2)
        b3 = QtWidgets.QPushButton('[N  ]', gb)
        b3.setFixedWidth(w)
        #b3 = PanelButton(text='[N  ]', parent=gb)
        b3.setCheckable(True)
        b3.setFlat(False)
        #bg.addButton(b3)
        ctrl.ui.connect_element_to_action(b3, 'set_bracketed_node_shape')
        hlayout.addWidget(b3)
        b4 = QtWidgets.QPushButton('[_]', gb)
        b4.setFixedWidth(w)
        #b4 = PanelButton(text='[_]', parent=gb)
        b4.setCheckable(True)
        b4.setFlat(False)
        #bg.addButton(b4)
        ctrl.ui.connect_element_to_action(b4, 'set_box_node_shape')
        hlayout.addWidget(b4)
        b5 = QtWidgets.QPushButton('( )', gb)
        b5.setFixedWidth(w)
        #b5 = PanelButton(text='( )', parent=gb)
