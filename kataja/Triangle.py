...
class Triangle(QtWidgets.QGraphicsItem, FadeInOut):
    ...
    def paint(self, painter, option, widget=None):
        ...
        if simple:
            ...
            shape_name = ctrl.forest.settings.get_edge_setting('shape_name', edge_type=edge_type)
            path_class = SHAPE_PRESETS[shape_name]
            path, lpath, foo, bar = path_class.path((center, top), (right, bottom), [], g.BOTTOM, g.TOP)
            fill = path_class.fillable and ctrl.forest.settings.get_shape_setting('fill', edge_type=edge_type)
            if fill:
