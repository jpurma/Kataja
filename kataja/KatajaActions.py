...
class KatajaActions:
    ...
    def next_structure(self):
        ...
        ctrl.ui.clear_items()
        self.add_message('(.) tree %s: %s' % (i + 1, ctrl.forest.textual_form()))
    ...
    def toggle_traces(self):
        ...
        if ctrl.forest.settings.traces_are_grouped_together() and not ctrl.forest.settings.uses_multidomination():
            ctrl.forest.settings.uses_multidomination(True)
            ctrl.forest.settings.traces_are_grouped_together(False)
            self.add_message('(t) use multidominance')
            ctrl.forest.traces_to_multidomination()
        elif (
                not ctrl.forest.settings.traces_are_grouped_together()) and not ctrl.forest.settings.uses_multidomination():
            ctrl.forest.settings.uses_multidomination(False)
            ctrl.forest.settings.traces_are_grouped_together(True)
            self.add_message('(t) use traces, group them to one spot')
            ctrl.forest.group_traces_to_chain_head()
        elif ctrl.forest.settings.uses_multidomination():
            ctrl.forest.settings.uses_multidomination(False)
            ctrl.forest.settings.traces_are_grouped_together(False)
