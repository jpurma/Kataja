...
from kataja.plugins.StacklessShifter.Parser import read_lexicon, linearize
...
class Document(KatajaDocument):
    ...
    def create_forests(self, filename=None, treelist=None, clear=False):
        ...
        if LEARN:
            self.preload_all_sentences(sentences)
        else:
    ...
    def preload_all_sentences(self, sentences):
        parser = classes.SyntaxAPI().parser
        parser.lexicon = self.lexicon
        for sentence in sentences:
            if sentence.strip().startswith('['):
                parser.analyze_and_make_features(sentence)
        print(f'Preloaded {len(sentences)} sentences, created lexicon with {len(self.lexicon)} items. ')
