...
from kataja.singletons import prefs, qt_prefs, ctrl, running_environment
...
class PluginSelector(QtWidgets.QWidget):
    ...
    def prepare_plugins_selection_widget(self):
        ...
        for key in sorted(ctrl.main.available_plugins.keys()):
            ...
            enabled.clicked.connect(self.enable_plugin)
    ...
    def refresh_plugin_selection(self):
        ctrl.main.find_plugins(running_environment.plugins_path)
        self.inner_widget = self.prepare_plugins_selection_widget()
    ...
    def enable_plugin(self, value):
        button = self.sender()
        key = button.plugin_key
        if value:
