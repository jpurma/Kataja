        ...
        if isinstance(edge.start, ConstituentNode) and isinstance(edge.end, ConstituentNode):
            ...
            if left and right:
                if constituent_end is left:
                    syntax(
                        "Set child: This relation already exists: %s .left = %s" % (constituent_start, constituent_end))
                    return
                elif constituent_end is right:
                    syntax("Set child: This relation already exists: %s .right = %s" % (
                        constituent_start, constituent_end))
                    return
                else:
                    raise ForestSyntaxError(
                        "Trying to put child node %s, but parent node %s already has two children: (%s, %s)" % (
                            constituent_end, constituent_start, left, right))
