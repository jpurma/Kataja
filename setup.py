...
At the moment it supports building deployable standalone version for OS X. Requires Python3.5,
setuptools,
Qt5.6 and PyQt5 on the build machine, but the created app doesn't need them.
...
if sys.platform == 'darwin':
    ...
    for ef in list(frameworks):
        for fr in list(frameworks):
            file = app_contents + 'Resources/lib/python3.5/lib-dynload/PyQt5/' + ef + '.so'
    ...
    qt_plugins = ['imageformats/libqgif.dylib',
                  'mediaservice/libqavfcamera.dylib', 'imageformats/libqdds.dylib',
