...
var branch = function (sx, sy, ex, ey) {
    ctx.beginPath();
    var skew = 0.5;
...
var lbranch = function (sx, sy, bwidth, bheight) {
    var ex = sx - bwidth;
    var ey = sy + bheight;
    branch(sx, sy, ex, ey);
    return {x: ex, y: ey};
...
var rbranch = function (sx, sy, bwidth, bheight) {
    var ex = sx + bwidth;
    var ey = sy + bheight;
    branch(sx, sy, ex, ey);
    return {x: ex, y: ey};
...
var do_draw = function (start_x, start_y, r) {
    var my_bw = r * bwidth;
    var my_bh = r * bheight;
    l1.style.opacity = r;
    l2.style.opacity = r;
...
var update_colors = function (p) {
    ...
    sel = document.querySelectorAll(".sidebar a");
    var i;
...
var knob_click = function () {
    knob_r += 360 / color_sets.length;
    color_knob.style.transform = "rotate(' + knob_r + 'deg)";
    color_i += 1;
    if (color_sets.length === color_i) {
        color_i = 0;
    }
    update_colors(color_sets[color_i]);
};

var resize = function (skip_animation) {
...
var init = function () {
    ...
    if (draw_tree) {
        ...
        if (my_url.includes("#") || my_url.includes(".html") || my_url.includes("/documentation") || my_url.includes("/about") || my_url.includes("/download") || my_url.includes("/research")) {
            do_draw(100, 130, 1);
        } else {
